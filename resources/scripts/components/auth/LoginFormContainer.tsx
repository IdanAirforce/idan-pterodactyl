...
export default forwardRef<HTMLFormElement, Props>(({ title, subtitle, ...props }, ref) => (
    <Container>
        <div css={tw`text-center mb-6 px-2`}>
            <p css={tw`inline-flex items-center rounded-full px-3 py-1 text-xs tracking-[0.18em] uppercase text-blue-100 bg-blue-500/15 border border-blue-400/30`}>
                IdanDev Panel
            </p>
        ...
        <Form {...props} ref={ref}>
            ...
            >
                <div css={tw`absolute inset-0 pointer-events-none`}>
                    <div css={tw`absolute -top-24 -left-16 h-64 w-64 rounded-full bg-blue-500/15 blur-3xl`} />
                    <div css={tw`absolute -bottom-24 -right-14 h-64 w-64 rounded-full bg-indigo-500/15 blur-3xl`} />
                </div>
